...
src/baselines/     → deterministic policies; same interface as agents
...
- **New agent**: subclass `BaseAgent` in `src/agents/`, implement all abstract methods, add to `tasks.py` agent selection block
- **New baseline**: add policy to `src/baselines/baseline_policies.py`, register in `tasks.run_baseline_benchmark()` `baselines` dict
